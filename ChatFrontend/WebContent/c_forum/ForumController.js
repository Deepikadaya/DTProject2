myApp.controller("ForumController",function($scope,$rootScope,$location,$http)
{
	...
	function listForum()
	{
		...
		$http.get('http://localhost:8082/ChatMiddleware/ShowAllForum')
		.then(function(response)
		{
	...
	$scope.approve=function(forId)
	{
		...
		$http.get('http://localhost:8082/ChatMiddleware/approvedForum/'+forumId)
		.then(function(response)
				{
	...
	$scope.reject=function(forumId)
	{
		...
		$http.get('http://localhost:8082/ChatMiddleware/rejectForum/'+forumId)
		.then(function(response)
				{
	...
	$scope.deleteForum=function(forumId)
	{
		...
		$http.get('http://localhost:8082/ChatMiddleware/deleteForum/'+forumId)
		.then(function(response)
				{
